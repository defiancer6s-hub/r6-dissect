...
func (g *Generator) printHeader() {
	...
	g.printf("package %s\n", g.pkgName)
	g.printf("\n")
	g.printf("import \"github.com/rs/zerolog/log\"\n")
}

func (g *Generator) printGetter() {
	// unknown operators (e.g. ops newer than this table) must degrade the
	// output rather than abort the whole parse, so log and return the zero role
	g.printf("func (i Operator) Role() %s {\n", g.roleTypeName)
	g.printf("if r, ok := _operatorRoles[i]; ok {\n")
	g.printf("return r\n")
	g.printf("}\n")
	g.printf("log.Warn().Uint64(\"operatorID\", uint64(i)).Msg(\"role unknown for operator ID\")\n")
	g.printf("return \"\"\n")
	g.printf("}\n")
}
